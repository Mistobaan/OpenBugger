...
from openbugger.controllers.buggydatacreation import get_serialized_representation
from openbugger.models.gnn import GnnopenbuggerModel
from openbugger.models.visualize import predictions_to_html
from openbugger.mutations import ALL_REWRITE_SCOUTS, AbstractRewriteOp, ICodeRewriteScout, filter_ops_in_range
...
model_path = st.selectbox(
    "Model",
    options=glob(os.path.join(args.models_dir, "**", "*.pkl.gz"), recursive=True),
)
...
def predict(code_text):
    predictions = model.predict(
        get_data_iterator_from(code_text), nn, device, parallelize=False
    )
...
if len(code_text) > 0:
    ...
    st.components.v1.html(
        predictions_to_html(predictions, include_header=False),
        height=400,
        width=900,
        scrolling=True,
    )
    ...
    for original_data, _, _ in predictions:
        ...
        options = st.multiselect(
            "Visualized Edges",
            list(original_data["graph"]["edges"]),
            default=list(original_data["graph"]["edges"]),
        )
