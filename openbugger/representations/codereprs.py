...
from openbugger.representations.data import openbuggerGraph
from openbugger.utils.cstutils import PersistentMetadataWrapper, PositionFilter, subsumes_code_range
from openbugger.utils.text import get_text_in_range
...
class PythonCodeRelations:
    def __init__(self, code_text: str, path: Path):
        ...
        self.__ast_with_metadata_wrapper = PersistentMetadataWrapper(
            cst.parse_module(code_text), unsafe_skip_copy=True
        )

        self.__relations: DefaultDict[
            str, DefaultDict[TNode, Set[Tuple[TNode, Any]]]
        ...
        for node_rels in self.__relations.values():
            for from_node, target_nodes in node_rels.items():
                if (
                    ...
                    and isinstance(from_node, cst.CSTNode)
                    and from_node not in nodes_to_use
                ):
                    continue
                elif (
                    nodes_to_use is not None
                ...
                for node, metadata in target_nodes:
                    if isinstance(node, cst.CSTNode) and (
                        nodes_to_use is None or node in nodes_to_use
                    ):
                        add_node(node)
                    elif isinstance(node, DummyEntity) and (
                        target_range is None
                        or subsumes_code_range(node.range, target_range)
                    ):
                        add_node(node)
                    elif node is None:
                        assert (
                            metadata is not None
                        ), "Both the node and the metadata is empty."
                        add_node(metadata)
    ...
    ) -> Tuple[openbuggerGraph, Dict[Any, int]]:
        ...
        with open(filepath, "w") as f:
            ...
            for node, node_idx in node_idxs.items():
                ...
                if isinstance(node, (cst.CSTNode, DummyEntity)):
                    ...
                    f.write(
                        f'\t node{node_idx}[shape="rectangle", label="{node_lbl}"];\n'
                    )
                else:
            ...
            for token_node in token_nodes:
                ...
                f.write(
                    f'\t\tnode{node_idxs[token_node]}[shape="rectangle", label="{node_lbl}"];\n'
                )
            edge_color = edge_colors.get("NextToken", "black")
            self.__create_dot_edges(
                edge_color, f, node_idxs, self.__relations["NextToken"], "NextToken"
            )
            f.write("\t}\n")  # subgraph
    ...
    def __create_dot_edges(
        self, edge_color, f, node_idxs, nodes, rel_type, indent="\t", weight=None
    ):
        for from_node, to_nodes in nodes.items():
            from_idx = node_idxs.get(from_node)
            if from_idx is None:
            ...
            for node, metadata in to_nodes:
                ...
                if metadata is None or node is None:
                    f.write(
                        f'{indent}node{from_idx} -> node{to_idx} [label="{rel_type}" {edge_style}];\n'
                    )
                else:
                    f.write(
                        f'{indent}node{from_idx} -> node{to_idx} [label="{rel_type}.{metadata}" {edge_style}];\n'
                    )
