...
from openbugger.utils.cstutils import code_position_leq, is_whitespace_node, subsumes_code_range
        ...
        for pos, tokval in self.__token_nodes.items():
            if self.__prev_token is not None:
                self.__code_relations.add_relation(
                    "NextToken", self.__prev_token, tokval
                )
    ...
    def on_leave(self, original_node: cst.CSTNode) -> None:
        ...
        if (
            ...
            for t in token_pos_of_node:
                self.__token_nodes[t] = DummyEntity(
                    self.__tokens_to_visit[t], CodeRange(*t)
                )
                self.__code_relations.add_relation(
                    "Child", original_node, self.__token_nodes[t]
                )
