...
from libcst.metadata import CodeRange, PositionProvider, QualifiedNameProvider, QualifiedNameSource  # noqa: F401
...
class CallAndTypeInfo(cst.CSTVisitor):
    ...
    def __init__(
        self, code_relations: PythonCodeRelations, environment: Optional[Environment]
    ):
        super().__init__()
        self.__script = jedi.Script(
            code=code_relations.code_text,
            path=str(code_relations.path),
    ...
    def visit_Call(self, node: cst.Call):
        ...
        signatures: Sequence[Signature] = self.__script.get_signatures(
            line=pos.end.line, column=pos.end.column + 1
        )
        for candidate_signature in signatures:
            ...
            if len(docstring) > 0:
                self.__code_rels.add_relation(
                    "CandidateCallDoc", node, metadata=docstring
                )
        ...
        for i, arg in enumerate(node.args):
            ...
            for candidate_signature in signatures:
                ...
                self.__code_rels.add_relation(
                    "MayFormalName", arg, metadata=formal_param.name
                )
    ...
    @staticmethod
    def compute_call_and_type_info(
        code_rels: PythonCodeRelations, environment: Optional[Environment] = None
    ) -> None:
