...
from openbugger.representations.codereprs import PythonCodeRelations
...
class DataFlow(cst.CSTVisitor):
    ...
    def __init__(self, code_relations: PythonCodeRelations):
        ...
        self.__last_access: DefaultDict[
            QualifiedName, FrozenSet[cst.CSTNode]
        ] = defaultdict(frozenset)
        self.__last_write: DefaultDict[
            QualifiedName, FrozenSet[cst.CSTNode]
        ] = defaultdict(frozenset)
    ...
    def __record_access(self, node: cst.CSTNode) -> None:
        ...
        for qname in qual_names:
            self.__code_relations.add_relation(
                "OccurrenceOf", node, metadata=qname.name
            )
            ...
            for last_accessed_node in self.__last_access[qname]:
                self.__code_relations.add_relation(
                    "NextMayUse", last_accessed_node, node
                )
            for last_write_node in self.__last_write[qname]:
                self.__code_relations.add_relation(
                    "LastMayWrite", last_write_node, node
                )
    ...
    def _add_dummy_end_nodes(self) -> None:
        for qname, last_accesses in self.__last_access.items():
            for access in last_accesses:
                self.__code_relations.add_relation(
                    "MayFinalUseOf", access, metadata=qname.name
                )

    def __merge_last_access(
        self,
        accesses1: DefaultDict[str, FrozenSet[cst.CSTNode]],
        accesses2: DefaultDict[str, FrozenSet[cst.CSTNode]],
    ) -> DefaultDict[str, FrozenSet[cst.CSTNode]]:
        all_vars = set(accesses1) | set(accesses2)
        return defaultdict(
            frozenset,
            {v: frozenset(chain(accesses1[v], accesses2[v])) for v in all_vars},
        )
    ...
    def visit_If(self, node: cst.If) -> bool:
        ...
        self.__last_access, after_body_snapshot = (
            last_access_snapshot,
            self.__last_access,
        )
        if node.orelse is not None:
            node.orelse.visit(self)
        self.__last_access = self.__merge_last_access(
            self.__last_access, after_body_snapshot
        )
        return False
    ...
    def visit_Return(self, node: cst.Return) -> bool:
        ...
        self.__returns_from_access[-1] = self.__merge_last_access(
            self.__returns_from_access[-1], self.__last_access
        )
        self.__last_access = defaultdict(frozenset)
        return False

    def visit_Continue(self, node: cst.Continue) -> bool:
        self.__continue_from_access[-1] = self.__merge_last_access(
            self.__continue_from_access[-1], self.__last_access
        )
        self.__last_access = defaultdict(frozenset)
        return False

    def visit_Break(self, node: cst.Break) -> bool:
        self.__break_from_access[-1] = self.__merge_last_access(
            self.__break_from_access[-1], self.__last_access
        )
        self.__last_access = defaultdict(frozenset)
        return False
    ...
    def visit_While(self, node: cst.While) -> bool:
        ...
        node.body.visit(self)
        self.__last_access = self.__merge_last_access(
            self.__continue_from_access.pop(), self.__last_access
        )
        ...
        self.__last_access = self.__merge_last_access(
            self.__last_access, accesses_after_first_test
        )
        if node.orelse is not None:
            node.orelse.visit(self)

        self.__last_access = self.__merge_last_access(
            self.__last_access, self.__break_from_access.pop()
        )
        return False
    ...
    def visit_For(self, node: cst.For) -> bool:
        ...
        self.__last_access = self.__merge_last_access(
            self.__last_access, accesses_after_zero_iterations
        )
        if node.orelse is not None:
            node.orelse.visit(self)
    ...
    def visit_FunctionDef(self, node: cst.FunctionDef) -> bool:
        ...
        node.body.visit(self)
        self.__last_access = self.__merge_last_access(
            self.__last_access, self.__returns_from_access.pop()
        )
        return False
