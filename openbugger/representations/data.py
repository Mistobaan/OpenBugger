...
from openbugger.utils.msgpackutils import load_msgpack_l_gz


class openbuggerGraph(TypedDict):
    nodes: List[str]
    ...
    def to_dot(
        cls,
        data: "openbuggerGraph",
        filepath: PathLike,
        edge_colors: Optional[Dict[str, str]],
    ) -> None:
        ...
        colored_nodes = [
            ...
            (
                "yellow",
                {target_idx for _, target_idx in data["edges"]["CandidateCallDoc"]},
            ),
        ]
        ...
        with open(filepath, "w") as f:
            ...
            for node_idx, node_lbl in enumerate(data["nodes"]):
                if node_idx in token_node_idxs:
                    continue
                node_specs = (
                    f'shape="rectangle", label="{node_idx}: {escape(node_lbl)}"'
                )
            ...
            for token_idx in token_node_idxs:
                ...
                node_specs = (
                    f'shape="rectangle", label="{token_idx}: {escape(node_lbl)}"'
                )
            ...
            for edge_name, edges in data["edges"].items():
                ...
                for edge in edges:
                    ...
                    f.write(
                        f'\tnode{from_idx} -> node{to_idx} [xlabel="{label}", {edge_style}];\n'
                    )
            f.write("}\n")  # graph
...
def add_open_vocab_nodes_and_edges(graph: openbuggerGraph) -> None:
    token_nodes = set()
...
class openbuggerData(TypedDict):
    graph: openbuggerGraph
    ...
    @classmethod
    def as_graph_data(cls, data: "openbuggerData") -> Tuple[GraphData, Optional[int]]:
        candidate_node_idxs, inv = np.unique(
            data["graph"]["reference_nodes"], return_inverse=True
        )
        if data["target_fix_action_idx"] is not None:
            target_node_idx = inv[data["target_fix_action_idx"]]
            assert (
                data["graph"]["reference_nodes"][data["target_fix_action_idx"]]
                == candidate_node_idxs[target_node_idx]
            )
        else:
        ...
        return (
            GraphData(
                ...
                edge_features={
                    e_type: [
                        e[2] if len(e) >= 3 else Vocabulary.get_pad() for e in adj_list
                    ]
                    for e_type, adj_list in data["graph"]["edges"].items()
                },
                reference_nodes={
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "output_path", type=str, help="the path to the target .dot file"
    )
    ...
    for i, data in enumerate(load_msgpack_l_gz(args.input_msgpack)):
        if i == args.idx:
            graph: openbuggerGraph = data["graph"]
            ...
            openbuggerGraph.to_dot(graph, args.output_path, EDGE_COLORS)
            break
    else:
