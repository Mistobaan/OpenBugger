...
class RemoveTypeAnnotations(cst.CSTTransformer):
    ...
    def leave_AnnAssign(
        ...
        qual_names: Set[QualifiedName] = self.get_metadata(
            QualifiedNameProvider, original_node.target
        )
    ...
    def leave_Param(
        ...
        if original_node.annotation is not None:
            qual_names: Set[QualifiedName] = self.get_metadata(
                QualifiedNameProvider, original_node.name
            )
            for qname in qual_names:
                self.annotations[qname.name] = original_node.annotation
...
if __name__ == "__main__":
    ...
    for qname, annotation in type_annotation_remover.annotations.items():
        print(qname, module.code_for_node(annotation.annotation))
