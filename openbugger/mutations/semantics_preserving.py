...
from libcst.metadata import QualifiedName, QualifiedNameProvider, QualifiedNameSource, Scope, ScopeProvider  # noqa:
...
class MirrorComparisons(cst.CSTTransformer):
    ...
    def __init__(
        self, p_swap: float = 1, rng: Optional[Random] = None, strict: bool = False
    ):
        super().__init__()
        if rng is None:
            self._rng = Random()
    ...
    def leave_Comparison(
        ...
        if any(
            isinstance(cmp.operator, (cst.Is, cst.IsNot, cst.In, cst.NotIn))
            for cmp in updated_node.comparisons
        ):
            return updated_node
        ...
        return cst.Comparison(
            ...
            comparisons=[
                cst.ComparisonTarget(
                    operator=self.get_mirrored(operator), comparator=comparator
                )
...
class _LocalVariablesFinder(cst.CSTVisitor):
    ...
    def visit_Name(self, node: cst.Name):
        ...
        if node.value in {"self", "cls"}:
            return
        fn_q_name: QualifiedName = tuple(
            self.get_metadata(QualifiedNameProvider, self.fn_cx[-1])
        )[0].name
        qual_names: Set[QualifiedName] = self.get_metadata(QualifiedNameProvider, node)
        for name in qual_names:
            if (
                name.name.startswith(fn_q_name + ".<locals>")
                and name.source == QualifiedNameSource.LOCAL
            ):
...
class RenameVariableInLocalScope(cst.CSTTransformer):
    ...
    CHAR_VOCAB: Final = (
        tuple(chr(i) for i in range(97, 97 + 26))
        + tuple(chr(i) for i in range(65, 65 + 26))
        + ("_",)
    )

    def __init__(
        self,
        qual_names: Dict[cst.FunctionDef, Set[str]],
        p_rename: float = 1.0,
    ...
    def visit_FunctionDef(self, node: cst.FunctionDef) -> bool:
        ...
        to_rename = self._rng.choices(
            tuple(self.__qual_names[node]), k=self._rng.randint(1, 3)
        )
        renamings = {}
        for q_name in to_rename:
            renamed_name = "".join(
                self._rng.choice(self.CHAR_VOCAB)
    ...
    def leave_Name(self, original_node: cst.Name, updated_node: cst.Name) -> cst.Name:
        ...
        if renamings is None:
            return updated_node
        qual_names: Set[QualifiedName] = self.get_metadata(
...
class NegateExpression(cst.CSTTransformer):
    ...
    def leave_Comparison(
        ...
        return cst.Comparison(
            ...
            comparisons=[
                cst.ComparisonTarget(
                    operator=self.OPPOSITES[type(comparison.operator)](),
    ...
    def on_leave(self, original_node, updated_node):
        ...
        if (
            ...
            updated_node = cst.UnaryOperation(
                operator=cst.Not(), expression=original_node
            )
        return updated_node
...
class IfElseSwap(cst.CSTTransformer):
    ...
    def leave_IfExp(
        ...
        return updated_node.with_changes(
            test=negated, body=updated_node.orelse, orelse=updated_node.body
        )
