...
from .rewrite_ops import AbstractRewriteOp, ArgSwapOp, ReplaceCodeTextOp
...
class ICodeRewriteScout(cst.BatchableCSTVisitor, ABC):
    ...
    def add_mod_op(
        self,
        op: AbstractRewriteOp,
        metadata: Tuple[Type["ICodeRewriteScout"], cst.CSTNode, Any],
    ) -> None:
...
class AssignRewriteScout(ICodeRewriteScout):
    ...
    def has_assignments_before_node(
        ...
        this_assignment_location: CodeRange = self.get_metadata(
            PositionProvider, this_assignment_node
        )

        for assignment in scope_data.assignments[name]:
            if isinstance(assignment, BuiltinAssignment):
                return True
            assignment_range: CodeRange = self.get_metadata(
                PositionProvider, assignment.node
            )
    ...
    def visit_Assign(self, node: cst.Assign):
        ...
        if isinstance(
            node.targets[0].target, cst.Name
        ) and not self.has_assignments_before_node(node.targets[0].target, node):
            return

        last_target_location: CodeRange = self.get_metadata(
            PositionProvider, node.targets[-1]
        )
        op_range = CodeRange(
            (
                last_target_location.end.line,
                last_target_location.end.column
                + len(node.targets[-1].whitespace_before_equal.value),
            ),
            (
                last_target_location.end.line,
                last_target_location.end.column
...
class BooleanOperatorRewriteScout(ICodeRewriteScout):
    boolean_op_replacements: Final = {
        cst.And: frozenset({cst.Or}),
        cst.Or: frozenset({cst.And}),
    }
        ...
        if (
            ...
            replace_op = ReplaceCodeTextOp(
                CodeRange(
                    ...
                    CodePosition(
                        operator_range.start.line, operator_range.start.column + 4
                    ),
                ),
                "",
            )
            ...
            replace_op = ReplaceCodeTextOp(
                CodeRange(test_range.start, test_range.start), "not "
            )
        else:
            return
...
class ComparisonOperatorRewriteScout(ICodeRewriteScout):
    ...
    def visit_ComparisonTarget(self, node: cst.ComparisonTarget):
        ...
        for op in target_swap_group:
            ...
            replace_op = ReplaceCodeTextOp(
                CodeRange(start_pos, end_pos),
                space_before + operator_token + space_after,
            )
...
class LiteralRewriteScout(ICodeRewriteScout):
    ...
    def visit_UnaryOperation(self, node: cst.UnaryOperation):
        try:
            if isinstance(node.operator, cst.Minus) and isinstance(
                node.expression, cst.Integer
            ):
                int_val = -int(node.expression.value)
                self.__replace_int_literal(int_val, node)
            elif isinstance(node.operator, cst.Plus) and isinstance(
                node.expression, cst.Integer
            ):
...
class VariableMisuseRewriteScout(ICodeRewriteScout):
    ...
    def __init__(
        self,
        op_store: List[AbstractRewriteOp],
        op_metadata_store: List[Tuple[Type["ICodeRewriteScout"], Any]] = None,
    ):
    ...
    def __is_local_symbol(self, scope: Scope, name: str) -> bool:
        return any(
            qn.source == QualifiedNameSource.LOCAL
    ...
    def visit_Name(self, node: cst.Name):  # noqa
        ...
        if not isinstance(scope_data, FunctionScope) and not isinstance(
            scope_data, ComprehensionScope
        ):
        ...
        def has_assignments_before_node(name: str):
            for assignment in scope_data.assignments[name]:
                assignment_node: cst.CSTNode = self.get_parent_statement(
                    assignment.node
                )
                assignment_location: CodeRange = self.get_metadata(
                    PositionProvider, assignment_node
                )
        ...
        for name in candidate_misuses:
            ...
            if not has_assignments_before_node(name):
                continue
            self.add_mod_op(
                ReplaceCodeTextOp(name_range, name),
                (VariableMisuseRewriteScout, node, candidate_misuses[name]),
            )
...
class ArgSwapRewriteScout(ICodeRewriteScout):
    ...
    def visit_Call(self, node: cst.Call):
        ...
        for i, j in combinations(non_param_args, 2):
            self.add_mod_op(
                ArgSwapOp(func_range, (i, j)), (ArgSwapRewriteScout, node, None)
            )
