...
class ReplaceCodeTextOp(AbstractRewriteOp):
    ...
    def rewrite(self, code: str) -> Tuple[str, "ReplaceCodeTextOp"]:
        assert (
            self.target_range.start.line == self.target_range.end.line
        ), "Cannot replace text on multiple lines."
            ...
            for line_no in count(start=1):
                ...
                if self.target_range.start.line == line_no:
                    ...
                    revert_op = ReplaceCodeTextOp(
                        replaced_text=next_input_line[
                            self.target_range.start.column : self.target_range.end.column
                        ],
                        target_range=CodeRange(
                            (
                                self.target_range.start.line,
                                self.target_range.start.column,
                            ),
                            (
                                self.target_range.start.line,
                                self.target_range.start.column
                                + len(self.replaced_text),
                            ),
                        ),
                    )
...
class ArgSwapOp(AbstractRewriteOp):
    idxs: Tuple[int, int] = field(
        default_factory=lambda x: (min(x[0], x[1], max(x[0], x[1])))
    )
    ...
    class _SwapListElements(cst.CSTTransformer):
        ...
        def leave_Call(
            ...
            node_range: CodeRange = self.get_metadata(
                PositionProvider, original_node.func
            )
    ...
    def rewrite(self, code: str) -> Tuple[str, "ArgSwapOp"]:
        ...
        assert (
            swap_visitor.target_found
        ), "The target target_range of the rewrite was not found."
