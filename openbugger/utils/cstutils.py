...
def code_position_within_range(pos: CodePosition, target_range: CodeRange) -> bool:
    ...
    return code_position_leq(target_range.start, pos) and code_position_leq(
        pos, target_range.end
    )
...
def subsumes_code_range(smaller: CodeRange, bigger: CodeRange) -> bool:
    ...
    """
    return code_position_within_range(
        smaller.start, bigger
    ) and code_position_within_range(smaller.end, bigger)
...
def relative_range(base: CodeRange, target: CodeRange) -> CodeRange:
    ...
    return CodeRange(
        (relative_start_line_no, relative_start_col_no),
        (relative_end_line_no, relative_end_col_no),
    )
