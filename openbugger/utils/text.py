        ...
        for line_no in count(start=1):
            ...
            if range.start.line <= line_no <= range.end.line:
                if range.start.line == range.end.line:
                    output_sb.write(
                        next_input_line[range.start.column : range.end.column]
                    )
...
def text_to_range_segments(
    ...
    def non_empty(code_range: CodeRange):
        if code_range.start == code_range.end:
            return CodeRange(
                code_range.start,
                CodePosition(code_range.end.line, code_range.end.column + 1),
            )
        return code_range
    ...
    ranges.append(
        (
            get_text_in_range(
                text_segment, CodeRange(current_pos, CodePosition(math.inf, math.inf))
            ),
            set(),
        )
    )
