...
class LatencyRecorder:
    ...
    def start(self):
        assert (
            not self._running
        ), "Tried to start a latency recorder that is already running."
        self._running = True
...
class MetricProvider:
    ...
    _histogram_boundaries = list(np.linspace(start=0.001, stop=128, num=512)) + [
        256,
        512,
    ]
    ...
    def __init__(
        ...
        module_name: str,
        push: bool = False,
        push_gateway_address: str = "localhost:9091",
    ):
        self._module_name = (
            module_name.replace(".", "_") + "_"
        )  # Grafana cannot handle '.' in metric names.
    ...
    def start_server(self, port: int = 8000) -> None:
        assert (
            not self._push
        ), "Should not start server if we are using the push gateway."
        try:
    ...
    def set_push_gateway_address(self, address: str) -> None:
        assert (
            self._push
        ), "Trying to set a push gateway address when not using push metrics."
    ...
    def new_counter(self, counter_name: str, description: str = "") -> Counter:
        self._check_ready()
        counter = Counter(
            name=self._module_name + counter_name,
    ...
    def new_gauge(self, gauge_name: str, description: str = "") -> Gauge:
        self._check_ready()
        return Gauge(
            name=self._module_name + gauge_name,
            documentation=description,
            registry=self._registry,
        )

    def new_queue(
        self, queue_name: str, maxsize=0, description: str = ""
    ) -> MetredQueue:
        self._check_ready()
        gauge = self.new_gauge(gauge_name=queue_name, description=description)
        return MetredQueue(gauge=gauge, maxsize=maxsize)

    def new_latency_measure(
        self, measure_name: str, description: str = ""
    ) -> LatencyRecorder:
        self._check_ready()
