...
from openbugger.controllers.datageneratingpipeline_coordinator import data_pipeline_proxy
from openbugger.representations.data import openbuggerData
from openbugger.utils.cstutils import relative_range
...
def get_data_from_folder(
    ...
    for sample in tqdm(load_all_msgpack_l_gz(RichPath.create(path))):
        sample: openbuggerData

        fn_name = f'{sample["graph"]["path"]}:{sample["graph"]["code_range"][0][0]}-{sample["graph"]["code_range"][0][1]}'

        if fn_name != current_fn and len(fn_samples) > 0:
            yield {
                "original": fn_samples["NO_BUG"],
                "rewrites": {
                    str(p): (s, 1 / len(fn_samples)) for p, s in fn_samples.items()
                },
            }
        ...
        if sample["target_fix_action_idx"] is None:
            ...
            inverse_rewrite = sample["candidate_rewrites"][
                sample["target_fix_action_idx"]
            ]
            inverse_rewrite_range = sample["candidate_rewrite_ranges"][
                sample["target_fix_action_idx"]
            ]

            if inverse_rewrite[0] == "ReplaceText":
                inverse_op_range = CodeRange(
                    tuple(inverse_rewrite_range[0]), tuple(inverse_rewrite_range[1])
                )
                snippet_range = sample["graph"]["code_range"]
                relative_r = relative_range(
                    CodeRange(tuple(snippet_range[0]), tuple(snippet_range[1])),
                    inverse_op_range,
                )
                ...
                for idx, ((rewrite_op, rewrite_args), rewrite_range) in enumerate(
                    zip(
                        fn_samples["NO_BUG"]["candidate_rewrites"],
                        fn_samples["NO_BUG"]["candidate_rewrite_ranges"],
                    )
                ):
                    if rewrite_op != "ReplaceText":
                        continue
                    if (
                        rewrite_range[0] != inverse_rewrite_range[0]
                    ):  # Rewrites should start at the same position.
                        continue
...
if __name__ == "__main__":
    ...
    proxy_thread.start()
    while True:
        publish_data_from_folder(
            args.data_path, "tcp://localhost:5557", args.throttle_elements_sent_per_sec
        )
