...
from openbugger.utils import detect_encoding_and_open
from openbugger.utils.cstutils import AllFunctionFinder
from openbugger.utils.logging import configure_logging

from ...controllers.buggydatacreation import get_serialized_representation
from ...models.gnn import GnnopenbuggerModel
from ...mutations import ALL_REWRITE_SCOUTS, AbstractRewriteOp, ICodeRewriteScout, filter_ops_in_range
from ...representations.coderelations import compute_all_relations
from ...representations.codereprs import PythonCodeRelations
from ...utils import run_and_debug
...
def get_data_iterator_from(
    ...
    rel_db.ast_with_metadata_wrapper.visit_batched(
        [
            ScoutClass(available_ops, available_ops_metadata)
            for ScoutClass in ALL_REWRITE_SCOUTS
        ]
    )
    ...
    for fn_node, fn_range in function_finder.all_function_nodes:
        if not report_uncovered and not any(
            fn_range.start.line <= l <= fn_range.end.line for l in covered_lines
        ):
            # Not covered.
            continue

        relevant_ops, relevant_op_metadata = filter_ops_in_range(
            available_ops, available_ops_metadata, fn_range
        )
...
def run(arguments):
    ...
    test_coverage = {
        f: d
        for f, d in test_coverage.items()
        if str(code_location) in str(Path(f).absolute())
    }
    print(f"{len(test_coverage)} files covered by the tests")

    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    model_path = Path(arguments.model_path)
    model, nn = GnnopenbuggerModel.restore_model(model_path, device)
    ...
    for filepath, coverage_data in tqdm(test_coverage.items()):
        ...
        for covered_fn_data, relevant_ops in get_data_iterator_from(
            ...
            predicted_node_idx = max(
                location_logprobs, key=lambda k: location_logprobs[k]
            )
            ...
            for rewrite_idx, (rewrite_node_idx, rewrite_logprob) in enumerate(
                ...
                if (
                    ...
                    and rewrite_logprob > predicted_rewrite_logprob
                ):
            ...
            if (
                np.exp(prediction_logprob + predicted_rewrite_logprob)
                < confidence_threshold
            ):
                continue

            op = relevant_ops[predicted_rewrite_idx]
            assert covered_fn_data["candidate_rewrites"][predicted_rewrite_idx] == (
                op.op_name(),
                op.rewrite_data(),
            )
            assert covered_fn_data["candidate_rewrite_ranges"][
                predicted_rewrite_idx
            ] == (
            ...
            if not any(
                op.target_range.start.line <= l <= op.target_range.end.line
                for l in coverage_data["executed_lines"]
            ):
                if arguments.report_uncovered:
                    num_warnings_unfiltered += 1
                    print(
                        f'Testing at range {covered_fn_data["graph"]["code_range"]} in {filepath}...'
                    )
            ...
            try:
                ...
                if base_test_report != after_fix_test_report:
                    ...
                    print(f"\tApplying candidate bug fix {op}")
                    print(
                        "\tTest Suite could not filter bug fix suggestions. Potential bug:"
                    )
                    print(f"\t Predicted rewrite: {op}")
                    print("\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "package_path", type=str, help="the directory where the package is located."
    )
    parser.add_argument(
        "code_dir",
        type=str,
        help="the subdirectory within the package that contains the actual code.",
    )

    parser.add_argument(
        "model_path", type=str, help="the path to the openbugger model."
    )
    parser.add_argument(
        "--confidence-level",
        type=float,
    ...
    parser.add_argument(
        "--report-uncovered",
        action="store_true",
        help="Report raised warnings when they are uncovered by tests.",
    )
