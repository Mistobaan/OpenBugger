...
from openbugger.controllers.helper.dummydatageneratingpipeline import get_data_from_folder
...
from openbugger.utils.replaybuffer import ReplayBuffer
...
def run(args):
    ...
    if args.fixed_model_path is not None:
        ...
        model_sync_client = ModelSyncClient(
            args.model_server_address, do_not_update_before_sec=5 * 60
        )
    ...
    data_input_thread = Thread(
        target=lambda: queue_loader(replay_buffer, args.data_pipeline_address),
        daemon=True,
    )
    data_input_thread.start()

    hydration_thread = Thread(
        target=lambda: hydrate_replay_buffer(
            replay_buffer, args.initial_data_hydration_path, args.replay_buffer_size
        ),
        ...
        with tracer.start_as_current_span("Processing data point."):
            ...
            with tracer.start_as_current_span("Getting ready for scoring."):
                ...
                for rewrite_idx, (graph, _) in data["rewrites"].items():
                    ...
                    if graph is None:
                        LOGGER.error(
                            f"None element for graph. Rewrite_idx: {rewrite_idx}"
                        )
                        continue
                    graphs.append(graph)
                ...
                discriminator_rewrite_logprobs = [-math.inf] * (
                    num_rewrite_candidates + 1
                )  # +1 for the NO_BUG case

            with tracer.start_as_current_span("Scoring."):
                model_sync_client.update_params_if_needed(nn, model_device)
                predictions = model.predict(
                    graphs, nn, model_device, parallelize=not args.sequential
                )
                ...
                with tracer.start_as_current_span("Processing scores."):
                    for i, (
                        ...
                        if target_fix_action_idx is None:
                            ...
                            target_logprob = (
                                location_logprobs[ground_node_idx]
                                + rewrite_logprobs[target_fix_action_idx]
                            )
                ...
                original_graph[
                    "candidate_rewrite_logprobs"
                ] = discriminator_rewrite_logprobs
