...
from openbugger.data.modelsync import ModelSyncServer
from openbugger.models.modelregistry import load_model
from openbugger.models.train import construct_data_loading_callable
from openbugger.models.utils import LinearWarmupScheduler, optimizer
from openbugger.utils.iteratorutils import limited_queue_iterator
...
def run(arguments):
    ...
    training_data_queue = metric_provider.new_queue(
        "training_queue_from_buffer",
        maxsize=10000,
        description="The number of elements loaded from the data buffer.",
    )
    training_data = LazyDataIterable(
        lambda: limited_queue_iterator(training_data_queue, 200_000)
    )

    data_buffer_to_training_thread = Thread(
        target=lambda: training_queue_filler(
            training_data_queue, arguments.training_data_buffer_address
        ),
    ...
    validation_data = LazyDataIterable(
        lambda: load_all_msgpack_l_gz(validation_data_path)
    )
    ...
    trainer = ModelTrainer(
        ...
        minibatch_size=run_spec["training"].get(
            "minibatchSize", int(arguments.minibatch_size)
        ),
        optimizer_creator=lambda p: optimizer(
            p, float(run_spec["training"].get("learningRate", 1e-4))
        ),
    ...
    trainer.register_train_epoch_end_hook(
        lambda _, __, ___, metrics: train_loss_metric.record(metrics["Loss"])
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "model_config",
        type=str,
        help="the type of the model to train or a path to a .yaml config",
    )

    # TODO: For now use fixed validation data, later fix that...
    parser.add_argument(
        "validation_data_path", type=str, help="the path to fixed validation data."
    )

    parser.add_argument(
        "model_save_path", type=str, help="the target path to store the trained model."
    )
