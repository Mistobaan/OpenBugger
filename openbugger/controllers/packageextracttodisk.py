...
from openbugger.controllers.buggydatacreation import extract_for_package
from openbugger.data.deduplication import DuplicationClient
from openbugger.mutations import AssignRewriteScout, BinaryOperatorRewriteScout
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Orchestrator to extract (static graphs) across multiple packages."
    )
    parser.add_argument("package", type=str, help="The target package")
    parser.add_argument(
        "--target-dir",
        type=str,
        default="/data/targetDir",
        help="The path for the extracted data.",
    )
    parser.add_argument(
        "--debug",
        action="store_true",
        help="Enter debugging mode when an exception is thrown.",
    )
    parser.add_argument(
