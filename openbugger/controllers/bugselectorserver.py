...
from openbugger.controllers.helper.randombugselectorserver import random_bug_selector_server, select_random_rewrites
from openbugger.data.modelsync import MockModelSyncClient, ModelSyncClient
from openbugger.utils.logging import MetricProvider, configure_logging

LOGGER = logging.getLogger(__name__)
metric_provider = MetricProvider("BugSelectorServer")


def calculate_selection_distribution(
    logprobs: List[float], temperature: float = 1, epsilon: float = 0
) -> List[float]:
...
class BugSelectionStats:
    ...
    def add(self, sample, distribution, selected) -> None:
        ...
        self.uniform_baseline_entropy_sum += np.log(
            distribution.shape[0]
        )  # == -log(1/N)

        rewrites = Counter(
            rewrite_type for rewrite_type, _ in sample["candidate_rewrite_metadata"]
        )
        rewrites["NO_REWRITE"] = 1
        num_rewrites = sum(rewrites.values())

        for rewrite_type, rewrite_count in rewrites.items():
            self.available_rewrite_frequency[rewrite_type] += (
                rewrite_count / num_rewrites
            )
    ...
    def report(self) -> Dict:
        ...
        uniform_baseline_entropy = (
            self.uniform_baseline_entropy_sum / self.total_samples
        )
        ...
        return {
            "entropy": entropy,
            "uniform_baseline_entropy": uniform_baseline_entropy,
        }
...
def run(arguments):
    ...
    random_bug_selector_thread = Thread(
        target=lambda: random_bug_selector_server(
            arguments.bug_selector_server_address,
            terminate_random_bug_selector,
            context,
            socket,
        ),
    ...
    if args.fixed_model_path is not None:
        ...
        model_sync_client = ModelSyncClient(
            args.model_sync_server, do_not_update_before_sec=5 * 60
        )
    ...
    selection_distribution_entropy_gauge = metric_provider.new_gauge(
        "selection_entropy"
    )
    selection_distribution_uniform_entropy_gauge = metric_provider.new_gauge(
        "selection_uniform_entropy"
    )
    ...
    while True:
        ...
        with processing_timer:
            try:
                ...
                for rewrite_logprob, rewrite_reference_node_idx in zip(
                    ...
                    generator_rewrite_logprobs.append(
                        rewrite_logprob + location_logprobs[rewrite_reference_node_idx]
                    )
                ...
                rewrite_selection_distribution = calculate_selection_distribution(
                    logprobs=generator_rewrite_logprobs,
                    temperature=arguments.temperature_scaling,
                ...
                selected_rewrites = {
                    "NO_BUG"
                    if i == len(candidate_rewrites)
                    else str(i): float(generator_rewrite_logprobs[i])
                ...
                if "NO_BUG" not in selected_rewrites:
                    selected_rewrites["NO_BUG"] = float(
                        generator_rewrite_logprobs[len(candidate_rewrites)]
                    )
                stats.add(datapoint, rewrite_selection_distribution, selected_rewrites)

            except StopIteration:  # Emitted by predict
                selected_rewrites = select_random_rewrites(
                    data[0]["candidate_rewrites"]
                )
        ...
        if stats.total_samples % 200 == 199:
            ...
            selection_distribution_uniform_entropy_gauge.set(
                summary["uniform_baseline_entropy"]
            )
