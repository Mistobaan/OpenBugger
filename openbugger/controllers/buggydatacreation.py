...
from openbugger.data.deduplication import DuplicationClient
from openbugger.data.deduplication.tokenizers import python_dedup_tokenize_file
from openbugger.data.pypi.venv import create_venv_and_install
from openbugger.mutations import (
    ALL_REWRITE_SCOUTS,
    AbstractRewriteOp,
    ICodeRewriteScout,
...
from openbugger.representations.coderelations import compute_all_relations
from openbugger.representations.codereprs import DummyEntity, PythonCodeRelations
from openbugger.utils import call_with_timeout, detect_encoding_and_open
from openbugger.utils.cstutils import AllFunctionFinder, subsumes_code_range
from openbugger.utils.logging import LatencyRecorder, MetricProvider
...
def get_serialized_representation(
    ...
    if target_fix_op is not None:
        ...
        if target_action_idx == -1:
            LOGGER.error(
                "Found non reversible op for %s at %s (%s)",
                package_name,
                rel_db.path,
                target_range,
            )
    ...
    serializable, node_to_idx = rel_db.as_serializable(
        ...
        reference_nodes=[ref_node for _, ref_node, _ in available_rewrite_op_metadata],
    )
    ...
    return {
        "graph": serializable,
        "candidate_rewrites": [
            (op.op_name(), op.rewrite_data()) for op in available_rewrite_ops
        ],
        ...
        "candidate_rewrite_metadata": [
            (cls.__name__, metadata_as_serializable(n))
            for cls, _, n in available_rewrite_op_metadata
        ],
...
def apply_rewrite_and_create_sample(
    ...
    for function, fn_pos in function_finder.all_function_nodes:
        ...
        LOGGER.error(
            "Could not find target function for %s %s %s",
            reverse_op,
            filepath,
            package_name,
        )
    ...
    relevant_ops, relevant_op_metadata = filter_ops_in_range(
        available_ops, available_ops_metadata, target_fn_pos
    )
...
def extract_bugs_from(
    ...
    if num_semantics_preserving_transformations > 0:
        ...
        while len(input_snippets) - 1 <= num_semantics_preserving_transformations:
            ...
            try:
                input_snippets.add(
                    apply_semantics_preserving_transforms(original_code_text)
                )
            except Exception as e:
                LOGGER.exception(
                    "Error in applying semantics-preserving transforms", exc_info=e
                )
    ...
    for code_text in input_snippets:
        ...
        try:
            call_with_timeout(
                lambda: compute_all_relations(rel_db, jedi_env),
                timout_sec=compute_relations_timeout_sec,
            )
        except TimeoutError:
            LOGGER.error(
                "Computing Entity Relations in %s of `%s` timed out. Aborting.",
                filepath,
                package_name,
            )
            return
        ...
        available_ops: List[AbstractRewriteOp] = []
        available_ops_metadata: List[
            Tuple[Type[ICodeRewriteScout], cst.CSTNode, Any]
        ] = []
        rel_db.ast_with_metadata_wrapper.visit_batched(
            [
                ScoutClass(available_ops, available_ops_metadata)
        ...
        for function, fn_pos in function_finder.all_function_nodes:
            relevant_ops, relevant_op_metadata = filter_ops_in_range(
                available_ops, available_ops_metadata, fn_pos
            )
            if len(relevant_ops) > 5:  # To avoid introducing bias
                ...
                with request_latency_measure:
                    ...
                    selected_rewrites: Dict[Optional[str], float] = msgpack.loads(
                        socket.recv()
                    )

                function_graph_and_rewrites = {
                    "original": serialized_original_code,
                    "rewrites": {},
                }
                ...
                for rewrite_idx, rewrite_prob in selected_rewrites.items():
                    if rewrite_idx == "NO_BUG":
                        # No rewrite
                        function_graph_and_rewrites["rewrites"]["NO_BUG"] = (
                            serialized_original_code,
                            rewrite_prob,
                        )
                        continue
                    ...
                    try:
                        ...
                        function_graph_and_rewrites["rewrites"][rewrite_idx] = (
                            rewritten_graph,
                            rewrite_prob,
                        )
                    except Exception as e:
...
def should_extract(
    ...
    if deduplication_client is not None:
        if deduplication_client.check_if_duplicate_and_add(
            f"{package_name}::{file}",
            list(set(python_dedup_tokenize_file(filepath)["tokens"])),
        ):
