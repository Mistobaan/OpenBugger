...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Orchestrator to extract graphs and publish data."
    )
    parser.add_argument(
        "package_list_path",
        type=str,
    ...
    proxy_thread = Thread(
        target=data_pipeline_proxy, name="data_publishing_proxy", daemon=True
    )
    proxy_thread.start()
    ...
    while True:  # Keep publishing forever
        ...
        for package in all_packages:
            ...
            LOGGER.info(
                f"Worker `{worker_id}` asked for the next package to process. Sending `{package}`."
            )
            socket.send_string(package)
            package_counter.inc()

        duplication_index.clear()  # Reset duplication index
        LOGGER.info("All packages have been distributed. Restarting from scratch.")
