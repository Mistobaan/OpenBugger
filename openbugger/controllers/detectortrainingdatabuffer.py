...
def run(arguments):
    ...
    replay_buffer = ReplayBuffer(
        backing_dir=tmp.name, gauge=replay_gauge, ttl=arguments.sample_ttl
    )
    hydration_thread = Thread(
        target=lambda: hydrate_replay_buffer(
            replay_buffer, args.initial_data_hydration_path
        ),
    ...
    buffer_subscription_thread = Thread(
        target=lambda: connect_buffer_to_publisher(
            replay_buffer, arguments.data_pipeline_address
        ),
