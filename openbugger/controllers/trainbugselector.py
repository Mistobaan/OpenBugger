...
def run(arguments):
    ...
    queue_subscription_thread = Thread(
        target=lambda: bug_detector_scored_data_queue_server(
            training_queue, arguments.bug_detector_scored_data_server
        ),
        daemon=True,
    ...
    training_data = LazyDataIterable(
        lambda: limited_queue_iterator(
            training_queue, arguments.num_samples_between_eval
        )
    )
    ...
    model, nn, initialize_metadata = load_model(
        run_spec["selector"],
        model_path,
        arguments.restore_path,
        restore_if_model_exists=True,
    )
    ...
    def publish_created_model(model, nn, optimizer):
        ...
        model_sync_server = ModelSyncServer(
            arguments.model_sync_server_address, model, nn
        )
        model_publish_counter.inc()
    ...
    if initialize_metadata:
        ...
        trainer.load_metadata_and_create_network(
            data, parallelize=not arguments.sequential, show_progress_bar=True
        )
