...
def rewrite_is_cubert_wrong_operator_bug(
    ...
    old_operator = function_lines[fun_rewrite_line][
        rewritten_range[0][1] : rewritten_range[1][1]
    ].strip()
    new_operator = datapoint["candidate_rewrites"][rewrite_idx][1].strip()

    if (
        old_operator not in CUBERT_ALLOWED_OPERATORS
        or new_operator not in CUBERT_ALLOWED_OPERATORS
    ):
        return False
...
def translate_operator_example(datapoint: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    if not rewrite_is_cubert_wrong_operator_bug(
        datapoint, function_lines, picked_rewrite_idx
    ):
        return None
...
def translate_varmisuse_example(datapoint: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    if not rewrite_is_cubert_varmisuse_bug(
        datapoint, function_lines, picked_rewrite_idx
    ):
        return None
