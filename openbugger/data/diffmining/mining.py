...
from openbugger.utils.cstutils import PersistentMetadataWrapper
...
def get_rewrite_ops(code, start_line, end_line):
    ast_with_wrapper = PersistentMetadataWrapper(
        cst.parse_module(code), unsafe_skip_copy=True
    )
    ...
    relevant_ops, relevant_op_metadata = filter_ops_in_range(
        available_ops,
        available_ops_metadata,
        CodeRange((start_line, 0), (end_line, 999)),
    )
...
def get_simple_fixes(repo_url, max_size=5):
    ...
    for commit in miner.traverse_commits():
        ...
        for modification in (
            m for m in commit.modifications if m.filename.endswith(".py")
        ):
            ...
            if (
                len(added_lines - deleted_lines) != 0
                and len(deleted_lines - added_lines) != 0
            ):
                continue
            ...
            try:
                ...
                for rewrite_op, rewrite_op_metadata in zip(
                    relevant_ops, relevant_op_metadata
                ):
                    modified_code_text, reverse_op = rewrite_op.rewrite(
                        modification.source_code_before
                    )
                    if token_equals_without_comments(
                        modification.source_code, modified_code_text
                    ) and not token_equals_without_comments(
                        modification.source_code_before, modified_code_text
                    ):
                        yield {
                            "repo": repo_url,
                            "message": commit.msg,
...
if __name__ == "__main__":
    ...
    for i, sample in enumerate(
        ThreadedIterator(get_simple_fixes_from_repos(repos), max_queue_size=10000)
    ):
        print(
            f"------------------------ Sample {i}. Collected so far {total_collected} ---------------------------"
        )
