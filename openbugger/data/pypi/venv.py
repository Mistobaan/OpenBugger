...
def create_linux_venv(
    dir: Path, venv_location: Path, all_package_files: Path, package: str
) -> int:
    command = f"""#!/bin/bash
...
def create_venv_and_install(package: str) -> Iterator[PackageLabInfo]:
    with TemporaryDirectory() as dir:
        ...
        if os.name == "nt":
            returncode = create_windows_venv(
                dir, venv_location, all_package_files, package
            )
        else:
            returncode = create_linux_venv(
                dir, venv_location, all_package_files, package
            )
        ...
        with open(all_package_files) as f:
            ...
            for line in f.readlines():
                line = line.replace(
                    "\0", ""
                )  # Remove null chars. For Windows compatibility.
        ...
        yield PackageLabInfo(
            venv_location, pkg_location, package, pkg_version, all_files
        )
