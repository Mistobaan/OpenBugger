...
class ModelSyncClient:
    ...
    def update_params_if_needed(self, neural_net: nn.Module, device) -> bool:
        ...
        try:
            ...
            LOGGER.error(
                "Server did not respond. Continuing with the previous parameters."
            )
            return False
    ...
    def __ask_server(self, timeout_sec: int = 30):
        ...
        try:
            self.__socket.send(
                msgpack.dumps(
                    (self.__current_model_version, self.__current_param_version)
                )
            )
            if poller.poll(timeout_sec * 1000):
                ...
                self.__socket.connect(self.__server_address)
                raise TimeoutError(
                    f"Error asking for model update. Server did not reply in {timeout_sec} seconds."
                )
