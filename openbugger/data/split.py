...
def split_file(
    ...
    try:
        for datapoint in load_msgpack_l_gz(input_path):
            ...
            assert idx >= 0
            datapoint_provenance = datapoint_provenance[
                idx + len("/venv/lib/python3.8/site-packages") :
            ]
            library = datapoint["package_name"]
            file_set = get_fold(
                datapoint_provenance,
                library,
                train_ratio,
                valid_ratio,
                test_only_libraries,
...
if __name__ == "__main__":
    ...
    split_many_files(
        data_folder, output_folder, train_ratio, valid_ratio, test_only_libraries
    )
