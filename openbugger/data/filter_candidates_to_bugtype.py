...
from openbugger.data.filter_data_to_cubert import (
    BugType,
    rewrite_is_cubert_varmisuse_bug,
    rewrite_is_cubert_wrong_operator_bug,
)
from openbugger.utils.msgpackutils import load_msgpack_l_gz, save_msgpack_l_gz


def filter_datapoint_rewrites(
    datapoint: Dict[str, Any],
    rewrite_filter_fn: Callable[[Dict[str, Any], List[str], int], bool],
    ...
    result["graph"]["reference_nodes"] = list(
        compress(datapoint["graph"]["reference_nodes"], rewrite_mask)
    )
    result["candidate_rewrites"] = list(
        compress(datapoint["candidate_rewrites"], rewrite_mask)
    )
    result["candidate_rewrite_ranges"] = list(
        compress(datapoint["candidate_rewrite_ranges"], rewrite_mask)
    )
    result["candidate_rewrite_metadata"] = list(
        compress(datapoint["candidate_rewrite_metadata"], rewrite_mask)
    )
