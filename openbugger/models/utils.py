...
from openbugger.representations.data import TypeAnnotationData
from openbugger.utils.msgpackutils import load_all_msgpack_l_gz


def scatter_log_softmax(
    src: torch.Tensor, index: torch.Tensor, dim: int = -1, eps: float = 1e-12
) -> torch.Tensor:
    if not torch.is_floating_point(src):
        raise ValueError(
            "`scatter_log_softmax` can only be computed over tensors with floating point data types."
        )
    ...
    sum_per_index = torch.zeros_like(max_value_per_index).scatter_add_(
        dim=-1, index=index, src=recentered_scores.exp()
    )
...
def construct_type_training_data_callable(
    ...
    def data_it() -> Iterator[TypeAnnotationData]:
        ...
        for datum in load_all_msgpack_l_gz(
            ...
            if (
                limit_num_yielded_elements is not None
                and num_yielded > limit_num_yielded_elements
            ):
                return
...
def compute_generator_loss(
    ...
    unconditional_text_repair_logprobs = (
        text_localization_logprobs + text_repair_logprobs
    )
    generation_logprobs[text_rewrite_idxs] += unconditional_text_repair_logprobs
    # Var misuse:
    var_misuse_localization_logprobs = localization_logprobs[
        candidate_symbol_to_location_group
    ]
    unconditional_var_misuse_logprobs = (
        var_misuse_localization_logprobs + varmisuse_logprobs
    )
    generation_logprobs[candidate_rewrite_idxs] += unconditional_var_misuse_logprobs
    # Arg swap
    arg_swap_localization_logprobs = localization_logprobs[
        swapped_pair_to_call_location_group
    ]
    ...
    index = torch.cat((rewrite_to_graph_id, arrange)).masked_select(
        mask=observed_rewrites
    )
    observed_detection_logprobs = rewrite_logprobs.masked_select(mask=observed_rewrites)
    observed_generation_logprobs = generation_logprobs.masked_select(
        mask=observed_rewrites
    )
    if loss_type in ("norm-kl", "norm-rmse", "classify-max-loss"):
        ...
        if loss_type == "norm-rmse":
            renormalized_detection_logprobs = scatter_log_softmax(
                observed_detection_logprobs, index=index
            )

            # Re-normed probs should sum up to log(1)
            log_difference = torch.logaddexp(
                renormalized_detection_logprobs, observed_generation_logprobs
            )
            ...
            renormalized_failed_detection_logprobs = scatter_log_softmax(
                failed_detection_logprobs, index=index
            )
