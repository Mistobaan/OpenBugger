...
class GreatVarMisuseModule(ModuleWithMetrics):
    def __init__(
        ...
        encoded_vec = np.array(
            [
                pos / np.power(10000, 2 * i / embedding_dim)
        ...
        self.positional_encodings = nn.Parameter(
            torch.tensor(
                encoded_vec.reshape([1, 5000, embedding_dim]), dtype=torch.float32
            ),
            requires_grad=False,
        )
    ...
    def _module_metrics(self) -> Dict[str, Any]:
        return {
            ...
            "Localization Accuracy (NoBug)": (
                self.__localization_correct
                - self.__num_correctly_localized_buggy_samples
            )
    ...
    def forward(
        ...
        localization_loss = nn.functional.cross_entropy(
            localization_logits, error_locations
        )

        with torch.no_grad():
            localization_corrects = (
                localization_logits.argmax(dim=-1) == error_locations
            )
            self.__localization_correct += int(localization_corrects.sum())
            self.__num_correctly_localized_buggy_samples += int(
                (is_buggy * localization_corrects).sum()
            )

            self.__localization_loss_sum += (
                float(localization_loss) * localization_corrects.shape[0]
            )
        ...
        if num_buggy > 0:
            ...
            repair_logprobs = torch.logsumexp(
                pointer_logprobs.masked_fill(~buggy_targets, -math.inf), dim=-1
            )
            ...
            with torch.no_grad():
                repair_corrects = buggy_targets[
                    torch.arange(
                        buggy_targets.shape[0], device=repair_targets_mask.device
                    ),
    ...
    def compute_logprobs(
        ...
        embedded_input_sequence = self.__token_embedder(
            token_idxs=token_ids.reshape(-1, token_ids.shape[-1]),
            lengths=token_lengths.reshape(-1),
        ).reshape(
            token_ids.shape[0],
            token_ids.shape[1],
            self.__token_embedder.embedding_layer.embedding_dim,
        )  # [BatchSize, MaxLength, EmbeddingDim]

        embedded_input_sequence = (
            embedded_input_sequence
            + self.positional_encodings[:, : embedded_input_sequence.shape[1]]
        )

        state = embedded_input_sequence
        token_range = torch.arange(
            embedded_input_sequence.shape[1], device=embedded_input_sequence.device
        )
        token_mask = token_range.unsqueeze(0) > input_sequence_lengths.unsqueeze(
            -1
        )  # [BatchSize, MaxLength]
        for layer in self.__seq_layers:
            state = layer(state, token_mask, edges, edge_types)

        predictions_logits = self.__predictions(
            self.__ln_out(state)
        ...
        pointer_logprobs = torch.log_softmax(
            pointer_logits, dim=-1
        )  # [BatchSize, MaxLength]

        return is_buggy, localization_logits, pointer_logprobs


class GreatVarMisuse(
    AbstractNeuralModel[
        GreatRawDataPoint, TensorizedGreatDataPoint, GreatVarMisuseModule
    ]
):
    def __init__(
        self,
    ...
    def tensorize(
        self, datapoint: GreatRawDataPoint
    ) -> Optional[TensorizedGreatDataPoint]:
        ...
        return TensorizedGreatDataPoint(
            tokens=[
                self.__token_embedder.tensorize(t) for t in datapoint["source_tokens"]
            ],
            edges=np.array(
                [[e1, e2] for e1, e2, _, _ in datapoint["edges"]], dtype=np.int32
            ),
            edge_types=np.array(
                [
                    self.__edge_id_to_edge[etype]
                    for _, _, etype, _ in datapoint["edges"]
                ],
                dtype=np.int32,
            ),
    ...
    def extend_minibatch_with(
        self,
        tensorized_datapoint: TensorizedGreatDataPoint,
    ...
    def finalize_minibatch(
        ...
        input_sequence_lengths = np.array(
            [len(s.tokens) for s in samples], dtype=np.int32
        )
        max_seq_size = int(input_sequence_lengths.max())
        max_num_subtokens = self.__token_embedder.max_num_subtokens

        token_idxs = np.zeros(
            (len(samples), max_seq_size, max_num_subtokens), dtype=np.int32
        )
        ...
        for i, sample in enumerate(samples):
            ...
            edges.append(
                np.concatenate(
                    [np.full((len(sample.edges), 1), fill_value=i), sample.edges],
                    axis=-1,
                )
            )
        ...
        edge_types = np.concatenate(
            [edge_types, edge_types + len(self.__edge_id_to_edge)], axis=0
        )
        ...
        return {
            ...
            "input_sequence_lengths": torch.tensor(
                input_sequence_lengths, dtype=torch.int32, device=device
            ),
            ...
            "error_locations": torch.tensor(
                error_locations, dtype=torch.int64, device=device
            ),
            "repair_candidates_mask": torch.tensor(
                repair_candidates_mask, dtype=torch.bool, device=device
            ),
            "repair_targets_mask": torch.tensor(
                repair_targets_mask, dtype=torch.bool, device=device
            ),
        }
