...
class AbstractopenbuggerModel:
    ...
    def _init(self):
        self._target_rewrite_ops = Vocabulary.create_vocabulary(
            ...
            max_size=len(self.OPERATOR_REWRITES),
            count_threshold=0,
            add_unk=False,
        )
    ...
    def _compute_rewrite_data(
        self, datapoint: openbuggerData, candidate_node_idxs: List[int]
    ):
        ...
        if target_fix_action_idx is not None:
            target_node_idx = datapoint["graph"]["reference_nodes"][
        ...
        incorrect_node_id_to_text_rewrite_target: Dict[int, List[int]] = defaultdict(
            list
        )
        text_rewrite_original_idx: Dict[int, List[int]] = defaultdict(list)
        correct_text_rewrite_target: Optional[Tuple[int, int]] = None

        misused_node_ids_to_candidates_symbol_node_ids: Dict[
            int, List[int]
        ] = defaultdict(list)
        ...
        call_node_ids_to_candidate_swapped_node_ids: Dict[int, List[int]] = defaultdict(
            list
        )
        ...
        for i, (
            node_idx,
            (rewrite_type, rewrite_data),
            (rewrite_scout, rewrite_metadata),
        ) in enumerate(
            zip(
            ...
            if (
                self._tensorize_only_at_target_location_rewrites
                and node_idx != target_node_idx
            ):
            ...
            if rewrite_scout == "VariableMisuseRewriteScout":
                ...
                misused_node_ids_to_candidates_symbol_node_ids[node_idx].append(
                    rewrite_metadata
                )
        ...
        repr_location_group_ids = {
            location_node_idx: i
            for i, location_node_idx in enumerate(candidate_node_idxs)
        }
        ...
        def to_flat_node_selection(
            ...
            for repr_node_id, candidate_nodes in candidates.items():
                if (
                    correct_candidate is not None
                    and correct_candidate[0] == repr_node_id
                ):
                    correct_idx = len(candidate_node_ids) + correct_candidate[1]
                node_selection_repr_node_ids.extend(
                    repr_node_id for _ in candidate_nodes
                )
    ...
    def _iter_per_sample_results(
        ...
        text_repair_logprobs_per_group = logprobs_per_group(
            text_repair_logprobs, mb_data["rewrite_to_location_group"]
        )
        ...
        for sample_idx in range(num_samples):
            original_point: openbuggerData = original_datapoints[sample_idx]
            ...
            if node_mappings is not None:
                ...
                candidate_node_idxs = np.array(
                    [node_mappings[sample_idx][k] for k in candidate_node_idxs]
                )
            ...
            assert (
                len(localization_distribution[sample_idx])
                == len(candidate_node_idxs) + 1
            )

            location_logprobs = {
                node_idx: lprob
                for node_idx, lprob in zip(
                    candidate_node_idxs, localization_distribution[sample_idx]
                )
            }
            location_logprobs[-1] = localization_distribution[sample_idx][-1]

            text_rewrite_original_idx = mb_data["text_rewrite_original_idxs"][
                sample_idx
            ]
            candidate_rewrite_original_idx = mb_data["candidate_rewrite_original_idxs"][
                sample_idx
            ]
            pair_rewrite_original_idx = mb_data["pair_rewrite_original_idx"][sample_idx]

            (
                flat_arg_swap_logprobs,
                flat_text_repair_logprobs,
                flat_var_misuse_logprob,
            ) = ([], [], [])
            for _ in range(len(np.unique(original_point["graph"]["reference_nodes"]))):
                flat_arg_swap_logprobs.extend(
                    arg_swap_logprobs_per_group[next_location_group_idx]
                )
                flat_text_repair_logprobs.extend(
                    text_repair_logprobs_per_group[next_location_group_idx]
                )
                flat_var_misuse_logprob.extend(
                    varmisuse_logprobs_per_group[next_location_group_idx]
                )
