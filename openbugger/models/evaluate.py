...
from openbugger.models.gnn import GnnopenbuggerModel
from openbugger.utils.msgpackutils import load_all_msgpack_l_gz
...
def run(arguments):
    ...
    lim = (
        None
        if arguments["--limit-num-elements"] is None
        else int(arguments["--limit-num-elements"])
    )
    data = load_all_msgpack_l_gz(
        data_path, shuffle=True, limit_num_yielded_elements=lim
    )
    ...
    predictions = model.predict(
        data, nn, device, parallelize=not arguments["--sequential"]
    )

    num_samples, num_location_correct = 0, 0
    num_buggy_samples, num_repaired_correct, num_repaired_given_location_correct = (
        0,
        0,
        0,
    )
    ...
    for datapoint, location_logprobs, rewrite_probs in predictions:
        if arguments.get("--assume-buggy", False):
            ...
            norm = float(
                torch.logsumexp(torch.tensor(list(location_logprobs.values())), dim=-1)
            )
        ...
        if not sample_has_bug:
            ...
            ground_node_idx = datapoint["graph"]["reference_nodes"][
                target_fix_action_idx
            ]
            target_rewrite_scout = datapoint["candidate_rewrite_metadata"][
                target_fix_action_idx
            ][0]
            ...
            for rewrite_idx, (rewrite_node_idx, rewrite_logprob) in enumerate(
                ...
                if (
                    rewrite_node_idx == ground_node_idx
                    and rewrite_logprob > predicted_rewrite_logprob_given_location
                ):
            ...
            rewrite_given_location_is_correct = (
                predicted_rewrite_idx_given_location == target_fix_action_idx
            )
    ...
    print("==================================")
    print(
        f"Localization Accuracy {num_location_correct/num_samples:.2%} ({num_location_correct}/{num_samples})"
    )
    for scout_name, (num_correct, total) in sorted(
        localization_data_per_scout.items(), key=lambda item: item[0]
    ):
    ...
    if num_buggy_samples == 0:
        print(
            "--eval-only-no-bug is True. Repair Accuracy Given Location cannot be computed."
        )
    else:
        print(
    ...
    for scout_name, (num_correct, total) in sorted(
        repair_data_per_scout.items(), key=lambda item: item[0]
    ):
        print(f"\t{scout_name}: {num_correct/total:.1%}  ({num_correct}/{total})")
    ...
    detection_true_warnings = np.array(
        [
            has_bug and correct_location
            for _, has_bug, _, correct_location, _ in bug_detection_logprobs
        ]
    )
    ...
    false_discovery_rate = detection_false_warnings_up_to_threshold / (
        detection_true_warnings_up_to_threshold
        + detection_false_warnings_up_to_threshold
    )
    ...
    detection_precision = np.interp(
        threshold_x, thresholds[::-1], detection_precision[::-1], right=0
    )
    print("detection_precision = np." + repr(detection_precision))

    print("### Detection Recall ###")
    detection_recall = np.interp(
        threshold_x, thresholds[::-1], detection_recall[::-1], right=0
    )
    print("detection_recall = np." + repr(detection_recall))

    print("### Detection NO_BUG Precision ###")
    no_bug_precision = np.interp(
        threshold_x, thresholds[::-1], no_bug_precision[::-1], right=0
    )
    ...
    precision = true_warnings_up_to_threshold / (
        true_warnings_up_to_threshold + false_warnings_up_to_threshold
    )
    recall = true_warnings_up_to_threshold / sum(
        1 for _, has_bug, _, _, _ in bug_detection_logprobs if has_bug
    )
