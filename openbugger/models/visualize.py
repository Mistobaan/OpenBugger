...
from openbugger.utils import RichPath, run_and_debug
from openbugger.utils.iteratorutils import sampled_iterator
from openbugger.utils.msgpackutils import load_all_msgpack_l_gz
from openbugger.utils.text import text_to_range_segments

with open(
    os.path.join(
        os.path.split(__file__)[0], "visualizationassets", "annotated_snippet.mustache"
    )
) as f:
    ANNOTATED_SNIPPET_TEMPLATE = pystache.parse(f.read())
with open(
    os.path.join(os.path.split(__file__)[0], "visualizationassets", "document.mustache")
) as f:
    DOCUMENT_TEMPLATE = pystache.parse(f.read())


def predictions_to_html(  # noqa: C901
    predictions,
    ...
    for datapoint, location_logprobs, rewrite_logprobs in predictions:
        ...
        code_range = CodeRange(
            (code_range[0][0], code_range[0][1]), (code_range[1][0], code_range[1][1])
        )
        range_to_node = {}
        ...
        for node_idx, rewrite, node_range, rewrite_logprob in zip(
            ...
            node_range = CodeRange(
                (node_range[0][0], node_range[0][1]),
                (node_range[1][0], node_range[1][1]),
            )
        ...
        if target_fix_action_idx is None:
            ...
            target_action_range = datapoint["candidate_rewrite_ranges"][
                target_fix_action_idx
            ]
        ...
        with io.StringIO() as sb:
            ...
            for text_segment, ref_ranges in segments:
                ...
                for current_range in ref_ranges:
                    predicted_action, _ = max(
                        range_to_actions[current_range], key=lambda x: x[1]
                    )

                    is_ground_truth_range = target_action_range == current_range
                    is_predicted_range = (
                        range_to_node[current_range] == predicted_node_idx
                    )
                    range_logprob = (
                    ...
                    target_ranges_data.append(
                        {
                            ...
                            "rewrites": [
                                {
                                    "is_correct": is_ground_truth_range
                                    and rewrite == target_action,
                                    "is_predicted": is_ground_truth_range
                                    and rewrite == predicted_action,
                                    "rewrite": str(rewrite),
                                    "prob": f"{np.exp(rewrite_logprob):.1%}",
                                }
                                for rewrite, rewrite_logprob in range_to_actions[
                                    current_range
                                ]
                            ],
                ...
                snippet_data = {
                    ...
                    "contains_ground_range": any(
                        t["is_ground_range"] for t in target_ranges_data
                    ),
                    "contains_predicted_range": any(
                        t["is_predicted_range"] for t in target_ranges_data
                    ),
                }
                prediction_logprob = max(
                    prediction_logprob,
                    max(r["best_range_logprob"] for r in target_ranges_data),
                )
                sb.write(
                    renderer.render(ANNOTATED_SNIPPET_TEMPLATE, snippet_data).strip()
                )
    ...
    return renderer.render(
        DOCUMENT_TEMPLATE, dict(snippets=all_snippets, include_header=include_header)
    )
