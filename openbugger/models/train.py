...
) -> Callable[[], Iterator[openbuggerData]]:
...
def run(arguments):
    ...
    validation_data_path = RichPath.create(
    ...
    model, nn, initialize_metadata = load_model(
        model_spec, model_path, restore_path, arguments["--aml"]
    )
    ...
    trainer.register_train_epoch_end_hook(
        lambda model, nn, epoch, metrics: log_run(
            aml_ctx, "train", model, epoch, metrics
        )
    )
    trainer.register_validation_epoch_end_hook(
        lambda model, nn, epoch, metrics: log_run(
            aml_ctx, "valid", model, epoch, metrics
        )
    )

    if initialize_metadata:
        metadata_data_path = RichPath.create(
            arguments["TRAIN_DATA_PATH"], azure_info_path
        )
        data_for_metadata = LazyDataIterable(
            construct_data_loading_callable(
                metadata_data_path, shuffle=True, limit_num_yielded_elements=250_000
            )
        )
