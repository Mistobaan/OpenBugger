...
from openbugger.models.utils import compute_generator_loss, scatter_log_softmax, scatter_max
from openbugger.representations.data import openbuggerData

LOGGER = logging.getLogger(__name__)


class BaseTensorizedopenbuggerGnn(NamedTuple):
...
class GnnopenbuggerModule(ModuleWithMetrics):
    def __init__(
        self,
        ...
        self._text_repair_module = TextRepairModule(
            output_state_dim, rewrite_vocabulary_size
        )
    ...
    def __compute_gnn_output(self, graph_data):
        gnn_output: GnnOutput = self._gnn(
            **graph_data, return_all_states=self.__use_all_gnn_layer_outputs
        )
        if self.__use_all_gnn_layer_outputs:
            gnn_output = gnn_output._replace(
                output_node_representations=self.__summarization_layer(
    ...
    def compute_localization_logprobs(self, graph_data: Dict[str, Any]):
        ...
        (
            ...
            candidate_to_sample_idx=gnn_output.node_graph_idx_reference[
                "candidate_nodes"
            ],
    ...
    def _compute_repair_logprobs(
        ...
        if gnn_output.node_idx_references["varmisused_node_ids"].shape[0] > 0:
            ...
            varmisuse_logits = torch.zeros(
                0, device=gnn_output.node_idx_references["varmisused_node_ids"].device
            )
        ...
        if gnn_output.node_idx_references["call_node_ids"].shape[0] > 0:
            ...
            arg_swap_logits = torch.zeros(
                0, device=gnn_output.node_idx_references["call_node_ids"].device
            )
...
class GnnopenbuggerModel(
    AbstractNeuralModel[
        openbuggerData, BaseTensorizedopenbuggerGnn, GnnopenbuggerModule
    ],
    ...
    def update_metadata_from(self, datapoint: openbuggerData) -> None:
        graph_data, _ = openbuggerData.as_graph_data(datapoint)
        self.__gnn_model.update_metadata_from(graph_data)

    def build_neural_module(self) -> GnnopenbuggerModule:
        return GnnopenbuggerModule(
    ...
    def tensorize(
    ...
    ) -> Optional[BaseTensorizedopenbuggerGnn]:
        graph_data, target_location_node_idx = openbuggerData.as_graph_data(datapoint)
        ...
        ) = self._compute_rewrite_data(
            datapoint, graph_data.reference_nodes["candidate_nodes"]
        )
        ...
        assert (1 if correct_rewrite_target else 0) + (
            1 if correct_candidate_symbol_node else 0
        ) + (
        ...
        return BaseTensorizedopenbuggerGnn(
    ...
    def extend_minibatch_with(
        self,
        tensorized_datapoint: BaseTensorizedopenbuggerGnn,
        ...
        if tensorized_datapoint.target_location_node_idx is None:
            ...
            partial_minibatch["correct_candidate_node_idxs"].append(
                tensorized_datapoint.target_location_node_idx
                + partial_minibatch["mb_num_target_nodes"]
            )
        ...
        partial_minibatch["rewrite_to_graph_id"].extend(
            [graph_idx] * num_rewrite_candidates_in_datapoint
        )
    ...
    def finalize_minibatch(
        ...
        minibatch = {
            ...
            "correct_candidate_node_idxs": torch.tensor(
                accumulated_minibatch_data["correct_candidate_node_idxs"],
                dtype=torch.int64,
                device=device,
            ),
            "has_bug": torch.tensor(
    ...
    def predict(
        ...
        trained_nn: GnnopenbuggerModule,
        device,
        parallelize: bool,
