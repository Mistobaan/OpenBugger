...
from openbugger.models.utils import scatter_log_softmax, scatter_max
...
class LocalizationModule(ModuleWithMetrics):
    def __init__(
        ...
        self._summary_repr = nn.Linear(
            in_features=representation_size, out_features=representation_size, bias=True
        )
        self._l1 = nn.Linear(
            in_features=2 * representation_size,
            out_features=representation_size,
            bias=True,
        )
        self._repr_to_localization_score = nn.Linear(
            in_features=representation_size, out_features=1, bias=False
        )
    ...
    def _module_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "Weight of Buggy Samples": self._buggy_samples_weight_schedule(
                self._epoch_idx
            ),
        }
    ...
    def compute_localization_logprobs(
        ...
        sample_summary = scatter_max(
            self._summary_repr(candidate_reprs), index=candidate_to_sample_idx, dim=0
        )[0][candidate_to_sample_idx]
        l1_out = torch.sigmoid(
            self._l1(torch.cat([candidate_reprs, sample_summary], dim=-1))
        )
        candidate_scores = self._repr_to_localization_score(l1_out).squeeze(
            -1
        )  # [num_candidates]

        arange = torch.arange(
            num_samples, dtype=torch.int64, device=candidate_to_sample_idx.device
        )
    ...
    def forward(
        self, candidate_reprs, candidate_to_sample_idx, has_bug, correct_candidate_idxs
    ):
        (
            candidate_to_sample_idx,
            candidate_log_probs,
            arange,
        ) = self.compute_localization_logprobs(
        ...
        log_probs_per_sample = log_probs_per_sample.clamp(
            min=-math.inf, max=math.log(0.995)
        )

        if hasattr(self, "_abstain_weight") and self._abstain_weight > 0:
            log_probs_per_sample = log_probs_per_sample + torch.where(
                has_bug,
                self._abstain_weight
                * candidate_log_probs[arange + candidate_reprs.shape[0]],
        ...
        with torch.no_grad():
            ...
            self.__num_no_bug_correct += int(
                (has_bug.logical_not() * correct_samples).sum().cpu()
            )
