...
class MultiheadAttention(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self._out_proj = nn.Linear(
            value_dimension * num_heads, output_dimension, bias=False
        )
    ...
    def _compute_attention_probs(self, masked_elements, raw_attention_scores):
        raw_attention_scores = raw_attention_scores.transpose(
            2, 3
        )  # [B, query_len, num_heads, memory_len]
        raw_attention_scores = raw_attention_scores
        if masked_elements is not None:
            raw_attention_scores.masked_fill_(
                masked_elements.unsqueeze(1).unsqueeze(1), -math.inf
            )

        attention_probs = nn.functional.softmax(raw_attention_scores, dim=-1)
        attention_probs = self._dropout_layer(
            attention_probs
        )  # Odd, but exists in original BERT
