...
from openbugger.models.layers.multihead_attention import MultiheadAttention
    ...
    * If edges are present and edge_attention_bias_is_scalar=True,
    ...
    def forward(self, input_seq_states, masked_elements, edges, edge_types):
        ...
        attention_probs = self._compute_attention_probs(
            masked_elements, attention_scores
        )
        multiheaded_weighted_value_sum = self._compute_weighted_sum(
            values, attention_probs
        )
        if self._use_edge_value_biases:
    ...
    def _compute_edge_bias_scores(
        ...
        if self._edge_attention_bias_is_scalar:
            ...
            edge_bias_scores = torch.cat(
                [edge_attention_scores, r_edge_attention_scores]
            )  # [2 * num_edges, num_head]
        else:
    ...
    def _add_edge_value_biases(
        ...
        value_bias_per_r_edge = attention_probs[
            edge_sample_ids, edge_targets, :, edge_sources
        ].unsqueeze(-1) * self._reverse_edge_value_biases(edge_types).reshape(
            value_biases_shape
        )  # [num_edges, num_heads, value_dim]
