...
from openbugger.models.layers.mlp import MLP


class TextRepairModule(ModuleWithMetrics):
    def __init__(self, input_representation_size: int, rewrite_vocab_size: int):
        super().__init__()
        self.__text_rewrite_embeddings = nn.Embedding(
            rewrite_vocab_size, embedding_dim=input_representation_size
        )
    ...
    def compute_rewrite_logits(
        self, target_rewrite_node_representations, candidate_rewrites
    ):
        """
        :param target_rewrite_node_representations [N, D]
        :param candidate_rewrites: [N]
        """
        embedded_target_rewrites = self.__text_rewrite_embeddings(
            candidate_rewrites
        )  # [N, D]
        return self.__text_rewrite_scorer(
            torch.cat(
                (embedded_target_rewrites, target_rewrite_node_representations), dim=-1
            )
        ).squeeze(-1)
...
class SingleCandidateNodeSelectorModule(ModuleWithMetrics):
    ...
    def compute_per_slot_log_probability(
        self, slot_representations_per_target, target_nodes_representations
    ):
        """
        ...
        return self.__candidate_scorer(
            torch.cat(
                (slot_representations_per_target, target_nodes_representations), dim=-1
            )
        ).squeeze(-1)
    ...
    def forward(self, per_slot_logprobs, correct_symbol_node_idxs, selected_fixes=None):
        ...
        if selected_fixes is not None:
            with torch.no_grad():
                self.__num_correct += int(
                    selected_fixes[correct_symbol_node_idxs].sum()
                )
...
class CandidatePairSelectorModule(ModuleWithMetrics):
    ...
    def compute_per_pair_logits(
        self, slot_representations_per_pair, pair_representations
    ):
        """
        ...
        return self.__pair_scorer(
            torch.cat(
                (
                    ...
                    pair_representations.reshape(
                        pair_representations.shape[0], 2 * self._input_dim
                    ),
                ),
                dim=-1,
            )
