...
from openbugger.models.gnnlayerdefs import create_ggnn_mp_layers, create_mlp_mp_layers
from openbugger.models.seqmodel import SeqopenbuggerModel
...
def linear_warmdown(
    epoch_idx: int, num_warmdown_epochs: int, target_weight: float
) -> float:
    return max(target_weight, epoch_idx * (target_weight - 1) / num_warmdown_epochs + 1)


def buggy_sample_weight_schedule(
    weight_spec: Union[str, int, float]
) -> Callable[[int], float]:
    ...
    if warmdown:
        ...
        return partial(
            linear_warmdown,
            num_warmdown_epochs=num_warmdown_epochs,
    ...
    return GnnopenbuggerModel(
        GraphNeuralNetworkModel(
            ...
            message_passing_layer_creator=lambda n_edges: mp_layer(
                ...
                dropout_rate,
                n_edges,
                features_dimension=edge_feature_size,
            ),
        ...
        buggy_samples_weight_schedule=buggy_sample_weight_schedule(
            buggy_samples_weight_spec
        ),
    )
...
def construct_model_dict(
    gnn_constructor: Callable, seq_constructor: Callable
) -> Dict[str, Callable]:
    return {
        "gnn-mlp": lambda kwargs: gnn_constructor(
            mp_layer=create_mlp_mp_layers, add_self_edge=True, **kwargs
        ),
        "ggnn": lambda kwargs: gnn_constructor(
            mp_layer=create_ggnn_mp_layers, add_self_edge=False, **kwargs
        ),
        ...
        "seq-transformer": lambda kwargs: seq_constructor(
            layer_type="transformer", **kwargs
        ),
...
def load_model(
    ...
    assert model_path.name.endswith(
        ".pkl.gz"
    ), "MODEL_FILENAME must have a `.pkl.gz` suffix."
