...
class EnsembleWrapper:
    def __init__(
        self, models: List[AbstractopenbuggerModel], kind: Literal["avg", "consensus"]
    ):
        assert all(
            hasattr(m, "predict") for m in models
        ), "One of the models doesn't have a predict function."
        self._models = models
    ...
    def predict(
        ...
        for datapoint in data:
            sample_preds = [
                ...
                for d, model, nn in zip(
                    tee(data, len(self._models)), self._models, trained_nn.nns
                )
            ]
            ...
            if any(len(s) != 1 for s in sample_preds):
                LOGGER.warning(
                    "One of the ensemble members did not return a prediction: %s",
                    [len(s) for s in sample_preds],
                )
            ...
            if not hasattr(self, "_kind") or self._kind == "avg":
                (
                    ensemble_localization_probs,
                    ensemble_rewrite_probs,
                ) = self._avg_ensembling(
                ...
                member_predicted_locs = tuple(
                    max((l for l in loc_prob), key=lambda l: loc_prob[l])
                    for loc_prob in member_localization_probs
                )
                if all(
                    member_predicted_locs[0] == m for m in member_predicted_locs[1:]
                ):
                    (
                        ensemble_localization_probs,
                        ensemble_rewrite_probs,
                    ...
                    ensemble_localization_probs = {
                        loc: -math.inf for loc in member_localization_probs[0]
                    }
                    ensemble_localization_probs[
                        -1
                    ] = 0  # NO_BUG gets all the probability mass
                    ensemble_rewrite_probs = member_rewrite_probs[
                        0
    ...
    def _avg_ensembling(self, member_localization_probs, member_rewrite_probs):
        ...
        ensemble_localization_probs = {
            k: v + log_weight for k, v in member_localization_probs[0].items()
        }
        for member_localization_prob in member_localization_probs[1:]:
            for k, v in member_localization_prob.items():
                ensemble_localization_probs[k] = np.logaddexp(
                    ensemble_localization_probs[k], v + log_weight
                )
        ...
        for member_rewrite_prob in member_rewrite_probs[1:]:
            for i, v in enumerate(member_rewrite_prob):
                ensemble_rewrite_probs[i] = np.logaddexp(
                    ensemble_rewrite_probs[i], v + log_weight
                )
