...
from openbugger.models.basemodel import AbstractopenbuggerModel
from openbugger.models.layers.fixermodules import (
    CandidatePairSelectorModule,
    SingleCandidateNodeSelectorModule,
    TextRepairModule,
)
from openbugger.models.layers.localizationmodule import LocalizationModule
from openbugger.models.layers.relational_transformer import RelationalTransformerEncoderLayer
from openbugger.models.utils import compute_generator_loss, scatter_log_softmax
from openbugger.representations.data import openbuggerData, openbuggerGraph
...
class SeqopenbuggerModule(ModuleWithMetrics):
    def __init__(
        self,
        token_embedder: Union[
            TokenUnitEmbedder, SubtokenUnitEmbedder, CharUnitEmbedder
        ],
        embedding_dim: int,
        num_edge_types: int,
        num_layers: int,
        ...
        self.__positional_encoding = nn.Parameter(
            torch.randn(1, 5000, embedding_dim), requires_grad=True
        )
        ...
        self.__localization_module = LocalizationModule(
            ...
            buggy_samples_weight_schedule=buggy_samples_weight_schedule,
        )
        self._buggy_samples_weight_schedule = buggy_samples_weight_schedule

        self._text_repair_module = TextRepairModule(
            embedding_dim, rewrite_vocabulary_size
        )
    ...
    def _compute_repair_logprobs(
        ...
        if call_node_ids.shape[0] > 0:
            arg_swap_logits = self._argswap_module.compute_per_pair_logits(
                slot_representations_per_pair=output_representations[
                    call_node_ids[:, 0], call_node_ids[:, 1]
                ],
                pair_representations=output_representations[
                    candidate_swapped_node_ids[:, 0].unsqueeze(-1),
                    candidate_swapped_node_ids[:, 1:],
                ],
            )
        else:
        ...
        text_repair_logprobs, varmisuse_logprobs, arg_swap_logprobs = torch.split(
            logprobs,
            [
                text_repair_logits.shape[0],
                varmisuse_logits.shape[0],
                arg_swap_logits.shape[0],
            ],
        )
        return arg_swap_logprobs, text_repair_logprobs, varmisuse_logprobs

    def _compute_localization_logprobs(
        self, candidate_reprs, candidate_to_sample_idx, num_samples
    ):
        (
        ...
        **kwargs,  # Visualization data (ignored)
        ...
        if rewrite_logprobs is not None:
            ...
            text_repair_loss = self._text_repair_module(
                text_repair_logprobs, correct_rewrite_idxs
            )
            varmisuse_repair_loss = self._varmisuse_module(
                varmisuse_logprobs, correct_candidate_symbols
            )
            arg_swap_repair_loss = self._argswap_module(
                arg_swap_logprobs, correct_swapped_pair
            )

            buggy_samples_weight = self._buggy_samples_weight_schedule(self._epoch_idx)
            repair_loss = (
                text_repair_loss.sum()
                + varmisuse_repair_loss.sum()
                + arg_swap_repair_loss.sum()
            )
    ...
    def _compute_output_representation(
        ...
        token_range = torch.arange(
            input_sequence_ids.shape[1], device=token_sequence_lengths.device
        )
        token_mask = token_range.unsqueeze(0) < token_sequence_lengths.unsqueeze(
            -1
        )  # [B, max_seq_len]
        ...
        if self.__layer_type in {"rat", "transformer", "great"}:
            ...
            output_representation = (
                output_representation
                + self.__positional_encoding[:, : input_sequence_ids.shape[1]]
            )

            # The original BERT model has this layer after embedding + positional encoding
            output_representation = self.__dropout_layer(
                self.__input_layer_norm(output_representation)
            )
        ...
        if self.__layer_type in {"rat", "great"}:
            ...
            for layer in self.__seq_layers:
                output_representation = output_representation.transpose(
                    0, 1
                )  # [max_seq_len, B, D]
                output_representation = layer(
                    output_representation, src_key_padding_mask=seq_mask
                )
                output_representation = output_representation.transpose(
                    0, 1
                )  # [B, max_seq_len, D]
            ...
            output_representation, _ = self.__seq_layers(
                nn.utils.rnn.pack_padded_sequence(
                    output_representation,
                    lengths=token_sequence_lengths,
                    batch_first=True,
                    enforce_sorted=False,
                )
            )
            output_representation, lens = nn.utils.rnn.pad_packed_sequence(
                output_representation, batch_first=True
            )
        else:
            raise ValueError()
...
class SeqopenbuggerModel(
    AbstractNeuralModel[openbuggerData, SeqModelTensorizedSample, SeqopenbuggerModule],
    AbstractopenbuggerModel,
):
    def __init__(
        self,
    ...
    def __to_token_data(self, graph: openbuggerGraph):
        # Get token list
            ...
            while len(to_visit) > 0:
                ...
                if next_node_label == "ComparisonTarget":
                    for child_node_idx in children[next_node_idx]:
                        ...
                        assert (
                            False
                        ), "One of the children, should be a comparison operator."
                    ...
                    for child_node_idx in children[next_node_idx]:
                        ...
                        assert (
                            False
                        ), "One of the children, should be a binary operator."
        ...
        for symbol_node_idx, target_tokens in orignal_symbol_node_idxs.items():
            old_node_id_to_new[symbol_node_idx] = min(
                old_node_id_to_new[t] for t in target_tokens
            )
        ...
        for edge_type, adj_list in graph["edges"].items():
            if edge_type in NON_TOKEN_EDGES:
                continue
            new_edges[edge_type] = [
                (to_token_idx(from_idx), to_token_idx(to_idx))
                for from_idx, to_idx in adj_list
            ]
    ...
    def __extract_token_sequence(self, graph: openbuggerGraph) -> List[int]:
        ...
        if len(first_token) != 1:
            LOGGER.error(
                f"Encountered graph where the tokens are not connected in %s",
                graph["path"],
            )
            return []
    ...
    def update_metadata_from(self, datapoint: openbuggerData) -> None:
        try:
            token_data = self.__to_token_data(datapoint["graph"])
        except Exception as ex:
            LOGGER.exception(
                "Error in generating token sequence for %s",
                datapoint["graph"]["path"],
                exc_info=ex,
            )
            return None
    ...
    def build_neural_module(self) -> SeqopenbuggerModule:
        return SeqopenbuggerModule(
    ...
    def tensorize(
        self, datapoint: openbuggerData
    ) -> Optional[SeqModelTensorizedSample]:
        ...
        candidate_node_idxs, inv = np.unique(
            datapoint["graph"]["reference_nodes"], return_inverse=True
        )
        transformed_candidate_node_idxs = np.array(
            [old_node_id_to_new[n] for n in candidate_node_idxs]
        )
            ...
            assert (
                old_node_id_to_new[
                    datapoint["graph"]["reference_nodes"][
                        datapoint["target_fix_action_idx"]
                    ]
                ]
        ...
        target_rewrite_node_ids = [
            old_node_id_to_new[n] for n in target_rewrite_node_ids
        ]
        varmisused_node_ids = [old_node_id_to_new[n] for n in varmisused_node_ids]
        candidate_symbol_node_ids = [
            old_node_id_to_new[n] for n in candidate_symbol_node_ids
        ]
        call_node_ids = [old_node_id_to_new[n] for n in call_node_ids]
        candidate_swapped_node_ids = [
            (old_node_id_to_new[p1], old_node_id_to_new[p2])
            for p1, p2 in candidate_swapped_node_ids
        ]

        return SeqModelTensorizedSample(
            target_subtokens_ids=[
                self.__token_embedder.tensorize(t) for t in node_labels
            ],
    ...
    def extend_minibatch_with(
        self,
        tensorized_datapoint: SeqModelTensorizedSample,
        partial_minibatch: Dict[str, Any],
    ) -> bool:
        current_sample_idx = len(partial_minibatch["input_subtoken_ids"])
        partial_minibatch["input_subtoken_ids"].append(
            tensorized_datapoint.target_subtokens_ids
        )

        for edge_type, adj_list in tensorized_datapoint.intra_token_edges.items():
            partial_minibatch["edge_types"].extend(
                self.__edge_type_to_idx[edge_type] for _ in adj_list
            )
            partial_minibatch["edges"].extend(
                (current_sample_idx, f, t) for f, t in adj_list
            )

        partial_minibatch["has_bug"].append(
            tensorized_datapoint.target_location_idx is not None
        )
        ...
        partial_minibatch["candidate_location_idxs"].extend(
            (current_sample_idx, tid)
            for tid in tensorized_datapoint.candidate_location_idxs
        )
        ...
        if tensorized_datapoint.correct_rewrite_target is not None:
            partial_minibatch["correct_rewrite_idxs"].append(
                tensorized_datapoint.correct_rewrite_target
                + len(partial_minibatch["target_rewrites"])
            )
        partial_minibatch["target_rewrite_node_ids"].extend(
            (current_sample_idx, t)
            for t in tensorized_datapoint.target_rewrite_node_ids
        )
        partial_minibatch["target_rewrites"].extend(
            tensorized_datapoint.target_rewrites
        )
        partial_minibatch["rewrite_to_location_group"].extend(
            t + mb_num_repair_groups_offset
            for t in tensorized_datapoint.target_rewrite_to_location_group
        )
        partial_minibatch["text_rewrite_idxs"].extend(
            t + mb_rewrite_candidate_offset
            for t in tensorized_datapoint.text_rewrite_original_idx
        )
        num_rewrite_candidates_in_datapoint += len(
            tensorized_datapoint.text_rewrite_original_idx
        )
        ...
        partial_minibatch["candidate_symbol_node_ids"].extend(
            (current_sample_idx, t)
            for t in tensorized_datapoint.candidate_symbol_node_ids
        )
        partial_minibatch["candidate_symbol_to_location_group"].extend(
            t + mb_num_repair_groups_offset
            for t in tensorized_datapoint.candidate_symbol_to_varmisused_node
        )
        partial_minibatch["candidate_rewrite_idxs"].extend(
            t + mb_rewrite_candidate_offset
            for t in tensorized_datapoint.candidate_rewrite_original_idx
        )
        num_rewrite_candidates_in_datapoint += len(
            tensorized_datapoint.candidate_rewrite_original_idx
        )
        ...
        partial_minibatch["call_node_ids"].extend(
            (current_sample_idx, t) for t in tensorized_datapoint.call_node_ids
        )
        partial_minibatch["candidate_swapped_node_ids"].extend(
            (current_sample_idx, t1, t2)
            for t1, t2 in tensorized_datapoint.candidate_swapped_node_ids
        )
        partial_minibatch["swapped_pair_to_call_location_group"].extend(
            t + mb_num_repair_groups_offset
            for t in tensorized_datapoint.swapped_pair_to_call
        )
        partial_minibatch["pair_rewrite_idxs"].extend(
            t + mb_rewrite_candidate_offset
            for t in tensorized_datapoint.pair_rewrite_original_idx
        )
        num_rewrite_candidates_in_datapoint += len(
            tensorized_datapoint.pair_rewrite_original_idx
        )
        partial_minibatch[
            "mb_num_rewrite_candidates"
        ] += num_rewrite_candidates_in_datapoint

        partial_minibatch[
            "mb_num_repair_groups"
        ] += tensorized_datapoint.num_rewrite_locations_considered

        # Visualization info
        partial_minibatch["text_rewrite_original_idxs"].append(
            tensorized_datapoint.text_rewrite_original_idx
        )
        partial_minibatch["candidate_rewrite_original_idxs"].append(
            tensorized_datapoint.candidate_rewrite_original_idx
        )
        partial_minibatch["pair_rewrite_original_idx"].append(
            tensorized_datapoint.pair_rewrite_original_idx
        )

        # Rewrite logprobs
        partial_minibatch["rewrite_to_graph_id"].extend(
            [current_sample_idx] * num_rewrite_candidates_in_datapoint
        )
    ...
    def finalize_minibatch(
        ...
        input_sequence_ids = np.zeros(
            (num_samples, max_len, max_subtokens), dtype=np.int32
        )
        ...
        minibatch = {
            "input_sequence_ids": torch.tensor(
                input_sequence_ids, dtype=torch.int64, device=device
            ),
            "input_seq_num_subtokens": torch.tensor(
                input_seq_num_subtokens, dtype=torch.int64, device=device
            ),
            "token_sequence_lengths": torch.tensor(
                token_seq_lengths, dtype=torch.int64, device=device
            ),
            "edges": torch.tensor(
                accumulated_minibatch_data["edges"], dtype=torch.int64, device=device
            ),
            "edge_types": torch.tensor(
                accumulated_minibatch_data["edge_types"],
                dtype=torch.int64,
                device=device,
            ),
            "has_bug": torch.tensor(
                accumulated_minibatch_data["has_bug"], dtype=torch.bool, device=device
            ),
            "target_location_idxs": torch.tensor(
                accumulated_minibatch_data["target_location_idxs"],
                dtype=torch.int64,
                device=device,
            ),
            "candidate_location_idxs": torch.tensor(
                accumulated_minibatch_data["candidate_location_idxs"],
                dtype=torch.int64,
                device=device,
            ),
            ...
            "target_rewrite_node_ids": torch.tensor(
                accumulated_minibatch_data["target_rewrite_node_ids"],
                dtype=torch.int64,
                device=device,
            ),
            "target_rewrites": torch.tensor(
                accumulated_minibatch_data["target_rewrites"],
                dtype=torch.int64,
                device=device,
            ),
            "rewrite_to_location_group": torch.tensor(
                accumulated_minibatch_data["rewrite_to_location_group"],
                dtype=torch.int64,
                device=device,
            ),
            "correct_rewrite_idxs": torch.tensor(
                accumulated_minibatch_data["correct_rewrite_idxs"],
                dtype=torch.int64,
                device=device,
            ),
            # Var Misuse
            "varmisused_node_ids": torch.tensor(
                accumulated_minibatch_data["varmisused_node_ids"],
                dtype=torch.int64,
                device=device,
            ),
            "candidate_symbol_node_ids": torch.tensor(
                accumulated_minibatch_data["candidate_symbol_node_ids"],
                dtype=torch.int64,
                device=device,
            ),
            "candidate_symbol_to_location_group": torch.tensor(
                accumulated_minibatch_data["candidate_symbol_to_location_group"],
                dtype=torch.int64,
                device=device,
            ),
            "correct_candidate_symbols": torch.tensor(
                accumulated_minibatch_data["correct_candidate_symbols"],
                dtype=torch.int64,
                device=device,
            ),
            # Arg Swap
            "call_node_ids": torch.tensor(
                accumulated_minibatch_data["call_node_ids"],
                dtype=torch.int64,
                device=device,
            ),
            "candidate_swapped_node_ids": torch.tensor(
                accumulated_minibatch_data["candidate_swapped_node_ids"],
                dtype=torch.int64,
                device=device,
            ),
            "swapped_pair_to_call_location_group": torch.tensor(
                accumulated_minibatch_data["swapped_pair_to_call_location_group"],
                dtype=torch.int64,
                device=device,
            ),
            "correct_swapped_pair": torch.tensor(
                accumulated_minibatch_data["correct_swapped_pair"],
                dtype=torch.int64,
                device=device,
            ),
            # Visualisation info
            "text_rewrite_original_idxs": accumulated_minibatch_data[
                "text_rewrite_original_idxs"
            ],
            "candidate_rewrite_original_idxs": accumulated_minibatch_data[
                "candidate_rewrite_original_idxs"
            ],
            "pair_rewrite_original_idx": accumulated_minibatch_data[
                "pair_rewrite_original_idx"
            ],
            # Rewrite info
            "text_rewrite_idxs": torch.tensor(
                accumulated_minibatch_data["text_rewrite_idxs"],
                dtype=torch.int64,
                device=device,
            ),
            "candidate_rewrite_idxs": torch.tensor(
                accumulated_minibatch_data["candidate_rewrite_idxs"],
                dtype=torch.int64,
                device=device,
            ),
            "pair_rewrite_idxs": torch.tensor(
                accumulated_minibatch_data["pair_rewrite_idxs"],
                dtype=torch.int64,
                device=device,
            ),
            # Rewrite logprobs
            "rewrite_to_graph_id": torch.tensor(
                accumulated_minibatch_data["rewrite_to_graph_id"],
                dtype=torch.int64,
                device=device,
            ),
        }
        ...
        if accumulated_minibatch_data["rewrite_logprobs"]:
            rewrite_logprobs = (
                accumulated_minibatch_data["rewrite_logprobs"]
                + accumulated_minibatch_data["no_bug_rewrite_logprobs"]
            )
            minibatch["rewrite_logprobs"] = torch.tensor(
                rewrite_logprobs, dtype=torch.float32, device=device
            )
    ...
    def predict(
        self,
        data: Iterator[openbuggerData],
        trained_nn: SeqopenbuggerModule,
        device,
        parallelize: bool,
    ) -> Iterator[Tuple[openbuggerData, Dict[int, float], List[float]]]:
        trained_nn.eval()
        with torch.no_grad(), self._tensorize_all_location_rewrites():
            for mb_data, original_datapoints in self.minibatch_iterator(
                self.tensorize_dataset(
                    data, return_input_data=True, parallelize=parallelize
                ),
                device,
                ...
                (
                    candidate_to_sample_idx,
                    candidate_log_probs,
                ) = trained_nn._compute_localization_logprobs(
                    target_location_representations,
                    candidate_location_idxs[:, 0],
                    num_samples,
                )
                ...
                (
                    ...
                    text_repair_logprobs,
                    varmisuse_logprobs,
                ) = trained_nn._compute_repair_logprobs(
                    output_representation,
